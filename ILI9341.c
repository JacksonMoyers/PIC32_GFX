...
/******************************************************************************
 * Description:  This function draws a vector and returns an ending x and y coordinate 
 * x: x position.  
 * y: y position.
 * distance: distance in km
 * head: heading as an int
 * 
 * Inputs: Start x and y coordinates, distance, and heading
 * 
 * Returns: int (x,y) ending x and y coordinates
 ******************************************************************************/
 void TFT_Vector(Uint x, Uint y, Uint distance, Uint heading)
{
   // int x1 =zoom*x-120;
   // int y1 =zoom*y-120;
    double head = heading* Pi/180;
    double x2=distance*cos(head);
    double y2 = distance*sin(head);
    double t = cos(head);
    double xfinal = 0;
    double yfinal = 0;
    if(x2 < 0)x2*=-1;
    ...
    TFT_Line(x,y,xfinal,y2,WHITE);
    sprintf(TFT_BUFF, "%.1f,%.1f", xfinal, yfinal);
